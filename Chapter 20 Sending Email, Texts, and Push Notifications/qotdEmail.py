...
import ezgmail, random, datetime, sys
...
#get the recipient
with open('recipient.txt', encoding='UTF-8') as f:
    RECIPIENT = f.read().strip()

random_quote = random.choice(quotes).strip()

ezgmail.send(RECIPIENT, 'Quote of the day', random_quote)
print(f'Sending an email to {RECIPIENT} with the following quote:\n\t{random_quote}')
